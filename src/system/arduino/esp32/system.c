...
/*------------------ Mutex ------------------*/
int8_t z_mutex_init(z_owned_mutex_t *m) { return pthread_mutex_init(&m->_val, NULL); }
...
int8_t z_mutex_try_lock(z_loaned_mutex_t *m) { return pthread_mutex_trylock(m); }

int8_t z_mutex_unlock(z_loaned_mutex_t *m) { return pthread_mutex_unlock(m); }

/*------------------ Condvar ------------------*/
int8_t z_condvar_init(z_owned_condvar_t *cv) { return pthread_cond_init(&cv->_val, NULL); }
