...
void _z_ring_mt_clear(_z_ring_mt_t *ring, z_element_free_f free_f) {
    ...
    z_condvar_free(&ring->_cv_not_empty);
#endif
...
#if Z_FEATURE_MULTI_THREAD == 1
    _Z_RETURN_IF_ERR(z_condvar_signal(&r->_cv_not_empty))
...
int8_t _z_ring_mt_pull(void *dst, void *context, z_element_move_f element_move) {
    ...
    while (src == NULL) {
        ...
        if (src == NULL) {
            _Z_RETURN_IF_ERR(z_condvar_wait(&r->_cv_not_empty, z_mutex_loan_mut(&r->_mutex)))
        }
    }
    _Z_RETURN_IF_ERR(z_mutex_unlock(z_mutex_loan_mut(&r->_mutex)))
