...
int8_t _z_ring_mt_init(_z_ring_mt_t *ring, size_t capacity) {
    ...
    _Z_RETURN_IF_ERR(z_mutex_init(&ring->_mutex))
    _Z_RETURN_IF_ERR(z_condvar_init(&ring->_cv_not_empty))
#endif
    return _Z_RES_OK;
}
...
#if Z_FEATURE_MULTI_THREAD == 1
    _Z_RETURN_IF_ERR(z_mutex_lock(z_mutex_loan_mut(&r->_mutex)))
#endif

    _z_ring_push_force_drop(&r->_ring, (void *)elem, element_free);

#if Z_FEATURE_MULTI_THREAD == 1
    _Z_RETURN_IF_ERR(z_condvar_signal(z_condvar_loan_mut(&r->_cv_not_empty)))
    _Z_RETURN_IF_ERR(z_mutex_unlock(z_mutex_loan_mut(&r->_mutex)))
#endif
    return _Z_RES_OK;
}
...
int8_t _z_ring_mt_pull(void *dst, void *context, z_element_move_f element_move) {
    ...
    while (src == NULL) {
        ...
        if (src == NULL) {
            _Z_RETURN_IF_ERR(z_condvar_wait(z_condvar_loan_mut(&r->_cv_not_empty), z_mutex_loan_mut(&r->_mutex)))
        }
    }
    _Z_RETURN_IF_ERR(z_mutex_unlock(z_mutex_loan_mut(&r->_mutex)))
