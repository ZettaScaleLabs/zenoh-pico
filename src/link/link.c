...
z_result_t _z_open_link(_z_link_t *zl, const _z_string_t *locator, const _z_config_t *session_cfg) {
    ...
    if (ret == _Z_RES_OK) {
        ...
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
            ret = _z_link_try_new_tcp(zl, &ep, session_cfg);
        }
#if Z_FEATURE_LINK_UDP_UNICAST == 1
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
            ret = _z_link_try_new_udp_unicast(zl, &ep, session_cfg);
        }
#endif
#if Z_FEATURE_LINK_BLUETOOTH == 1
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
            ret = _z_link_try_new_bt(zl, &ep, session_cfg);
        }
#endif
#if Z_FEATURE_LINK_SERIAL == 1
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
            ret = _z_link_try_new_serial(zl, &ep, session_cfg);
        }
#endif
#if Z_FEATURE_LINK_WS == 1
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
            ret = _z_link_try_new_ws(zl, &ep, session_cfg);
        }
#endif
#if Z_FEATURE_LINK_TLS == 1
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
            ret = _z_link_try_new_tls(zl, &ep, session_cfg);
        }
#endif
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
...
z_result_t _z_listen_link(_z_link_t *zl, const _z_string_t *locator, const _z_config_t *session_cfg) {
    ...
    if (ret == _Z_RES_OK) {
        ...
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
            ret = _z_link_try_new_udp_multicast(zl, &ep, session_cfg);
        }
#endif
#if Z_FEATURE_LINK_BLUETOOTH == 1
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
        ...
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
            ret = _z_link_try_new_raweth(zl, &ep, session_cfg);
        }
#endif
        if (ret == _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN) {
            _Z_ERROR_LOG(_Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN);
...
z_result_t _z_link_peer_copy(_z_link_peer_t *dst, const _z_link_peer_t *src) {
    if (dst == NULL) {
        _Z_ERROR_RETURN(_Z_ERR_INVALID);
    }
    *dst = _z_link_peer_null();
    if (!_z_link_peer_check(src)) {
        _Z_ERROR_RETURN(_Z_ERR_INVALID);
    }
    *dst = _z_link_peer_clone(src);
    if (!_z_link_peer_check(dst)) {
        _Z_ERROR_RETURN(_Z_ERR_SYSTEM_OUT_OF_MEMORY);
    }
    return _Z_RES_OK;
}
