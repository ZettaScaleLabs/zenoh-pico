...
static bool _z_endpoint_serial_matches(const _z_endpoint_t *endpoint) {
...
z_result_t _z_link_try_new_serial(_z_link_t *zl, _z_endpoint_t *ep, const _z_config_t *session_cfg) {
    ...
    if (!_z_endpoint_serial_matches(ep)) {
        return _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN;
    }
    _Z_RETURN_IF_ERR(_z_endpoint_serial_valid(ep));
    return _z_new_link_serial(zl, *ep);
}
#endif
