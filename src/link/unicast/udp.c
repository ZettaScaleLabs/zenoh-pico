...
static bool _z_endpoint_udp_unicast_matches(const _z_endpoint_t *endpoint) {
...
static z_result_t _z_f_link_peer_from_link_udp_unicast(const _z_link_t *link, _z_link_peer_t *peer) {
    ...
    return _z_link_peer_copy(peer, &link->_peer);
}
...
z_result_t _z_link_try_new_udp_unicast(_z_link_t *zl, _z_endpoint_t *ep, const _z_config_t *session_cfg) {
    ...
    if (!_z_endpoint_udp_unicast_matches(ep)) {
        return _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN;
    }
    _Z_RETURN_IF_ERR(_z_endpoint_udp_unicast_valid(ep));
    return _z_new_link_udp_unicast(zl, *ep);
}
#endif
