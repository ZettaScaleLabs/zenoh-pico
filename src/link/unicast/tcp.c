...
static bool _z_endpoint_tcp_matches(const _z_endpoint_t *endpoint) {
...
z_result_t _z_link_try_new_tcp(_z_link_t *zl, _z_endpoint_t *ep, const _z_config_t *session_cfg) {
    ...
    _Z_RETURN_IF_ERR(_z_endpoint_tcp_valid(ep));
    return _z_new_link_tcp(zl, ep);
}
#else
z_result_t _z_endpoint_tcp_valid(_z_endpoint_t *endpoint) {
    _ZP_UNUSED(endpoint);
...
z_result_t _z_link_try_new_tcp(_z_link_t *zl, _z_endpoint_t *ep, const _z_config_t *session_cfg) {
    ...
    if (_z_string_equals(&ep->_locator._protocol, &tcp_str)) {
        _Z_ERROR_RETURN(_Z_ERR_TRANSPORT_NOT_AVAILABLE);
    }
    return _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN;
}
