...
static bool _z_endpoint_tls_matches(const _z_endpoint_t *endpoint) {
...
z_result_t _z_link_try_new_tls(_z_link_t *zl, _z_endpoint_t *ep, const _z_config_t *session_cfg) {
    if (!_z_endpoint_tls_matches(ep)) {
        return _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN;
    }
    _Z_RETURN_IF_ERR(_z_endpoint_tls_valid(ep));
    return _z_new_link_tls(zl, ep, session_cfg);
}
