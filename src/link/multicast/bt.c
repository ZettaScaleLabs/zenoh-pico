...
static bool _z_endpoint_bt_matches(const _z_endpoint_t *ep) {
    _z_string_t bt_str = _z_string_alias_str(BT_SCHEMA);
    return _z_string_equals(&ep->_locator._protocol, &bt_str);
}

z_result_t _z_endpoint_bt_valid(_z_endpoint_t *ep) {
    if (!_z_endpoint_bt_matches(ep)) {
...
z_result_t _z_link_try_new_bt(_z_link_t *zl, _z_endpoint_t *ep, const _z_config_t *session_cfg) {
    ...
    _Z_RETURN_IF_ERR(_z_endpoint_bt_valid(ep));
    return _z_new_link_bt(zl, *ep);
}
#endif
