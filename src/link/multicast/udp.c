...
static bool _z_endpoint_udp_multicast_matches(const _z_endpoint_t *endpoint) {
...
z_result_t _z_endpoint_udp_multicast_valid(_z_endpoint_t *endpoint) {
    if (!_z_endpoint_udp_multicast_matches(endpoint)) {
...
z_result_t _z_new_link_udp_multicast(_z_link_t *zl, _z_endpoint_t endpoint) {
    ...
    z_result_t ret = _z_udp_multicast_endpoint_init_from_address(&state->_udp._rep, &endpoint._locator._address);
    if (ret != _Z_RES_OK) {
        z_free(state);
        return ret;
    }
...
z_result_t _z_link_try_new_udp_multicast(_z_link_t *zl, _z_endpoint_t *ep, const _z_config_t *session_cfg) {
    ...
    if (!_z_endpoint_udp_multicast_matches(ep)) {
        return _Z_ERR_CONFIG_LOCATOR_SCHEMA_UNKNOWN;
    }
    _Z_RETURN_IF_ERR(_z_endpoint_udp_multicast_valid(ep));
    return _z_new_link_udp_multicast(zl, *ep);
}
